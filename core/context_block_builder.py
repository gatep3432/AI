...
from persona.mood_tracker import get_current_mood, get_mood_summary  # 🔧 Removed apply_sentiment_to_mood import
from persona.hormone_adjuster import load_hormone_levels  # hormone level access
...
def build_turn_prompt(
    ...
    # 🔧 REMOVED: Duplicate mood processing (now only done in app.py)
...
[MINDSET]
Current Mood: {mood['current_mood']}{mood_type_str} (Intensity: {mood['intensity']:.2f})
Tone: {traits["tone"]}
Style: {traits["style"]}
Formality: {traits["formality"]}

[MOOD]
Mood: {mood['current_mood']}{mood_type_str}
Intensity: {mood['intensity']:.2f}
Hybrid: {is_hybrid}
Emergent: {is_emergent}
Stability: {stability}
Hormone Levels: {hormone_str}
Mood Context: {mood_context_desc}
